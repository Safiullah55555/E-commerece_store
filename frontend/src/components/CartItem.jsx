...
const CartItem = ({ item }) => {
        ...
        return (
                <div className="rounded-lg border p-4 shadow-sm border-gray-700 bg-wxhite md:p-6">
                        <div className="space-y-4 md:flex md:items-center md:justify-between md:gap-6 md:space-y-0">

                                <div className="shrink-0 md:order-1">
                                        <Link to={`/product/${item._id}`}>
                                                <img className='h-20 md:h-32 rounded object-cover cursor-pointer' src={item.image} alt={item.name} />
                                        </Link>
                                </div>
